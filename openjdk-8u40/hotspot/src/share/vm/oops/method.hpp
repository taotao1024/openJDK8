...
class Method : public Metadata {
 ...
 private:
  ...
  // 指向该Java方法的签名（signature）
  // 所对应的i2c2i adapter stub。当需要c2i
  //  adapter stub或i2c adapter stub的时候，调用
  // _adapter的get_i2c_entry()或get_c2i_entry()函
  // 数获取。
  AdapterHandlerEntry* _adapter;
  // Entry point for calling from compiled code, to compiled code if it exists
  // or else the interpreter.
  // _from_compiled_entry的
  // 初始值指向c2i adapter stub，也就是以编译模式
  // 执行的Java方法在调用需要以解释模式执行的Java
  // 方法时，由于调用约定不同，所以需要在转换时进
  // 行适配，而_from_compiled_entry指向这个适配的
  // 例程入口。一开始Java方法没有被JIT编译，需要
  // 在解释模式下执行。当该方法被JIT编译并“安
  // 装”完成之后，_from_compiled_entry会指向编译
  // 出来的机器码入口，具体说是指向verified entry
  //  point。如果要抛弃之前编译好的机器码，那么
  // _from_compiled_entry会恢复为指向c2i adapter stub。
  ...
  // _from_interpreted_entry的初始值与_i2i_entry
  // 一样，都是指向字节码解释执行的入口。但当该
  // Java方法被JIT编译并“安装”之后，
  // _from_interpreted_entry就会被设置为指向i2c
  //  adapter stub。如果因为某些原因需要抛弃之前已
  // 经编译并安装好的机器码，则
  // _from_interpreted_entry会恢复为指向
  // _i2i_entry。如果有_code，则通过
  // _from_interpreted_entry转向编译方法，否则通
  // 过_i2i_entry转向解释方法。
