...
void ThreadLocalAllocBuffer::make_parsable(bool retire) {
  if (end() != NULL) {
    ...
    //openjdk/hotspot/src/share/vm/gc_interface/collectedHeap.hpp#fill_with_object
...
void ThreadLocalAllocBuffer::initialize() {
  ...
  if (Universe::heap() != NULL) {
    // 在堆中支持分配的TLAB内存空间大小，因为只有Eden空间支持所以在Serial收集器
    // 中会返回Eden的内存空间
...
void ThreadLocalAllocBuffer::startup_initialization() {
  ...
  // 1/2 full at a GC
  // 选项-XX:TLABWasteTargetPercent设置浪费的TLAB可占用的Eden
  // 空间的百分比，默认值为1%，因此 TLABWasteTargetPercent 的值为1
