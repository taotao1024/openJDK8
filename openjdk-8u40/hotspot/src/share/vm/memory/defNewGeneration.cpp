    ...
    _should_allocate_from_space(false)
{
  ...
  _next_gen = NULL;
  // 控制新生代对象晋升到老年代中的最大阈值
  // 如果某个对象经历过_tenuring_threshold次GC后依然存活，则可以晋升到老年代；
...
void DefNewGeneration::compute_space_boundaries(uintx minimum_eden_size,
  ...
  // Compute sizes
  // committed_size()函数也就是实际上获取的是已经分配的物理内存
...
//   调用DefNewGeneration::allocate_from_space()
// 函数从From Survivor空间分配内存时，GC线程在执行
// 回收任务的时候需要开辟相应的辅助空间来完成内存的回收。
HeapWord* DefNewGeneration::allocate_from_space(size_t size) {
  ...
  //   支持在From Survivor空间中分配内存，或当前正在进行GC操作时可能会从From
  // Survivor空间中分配内存
  if (should_allocate_from_space() || GC_locker::is_active_and_needs_gc()) {
    // 当前线程拥有堆的全局锁 || 执行GC的VMThread线程借助From Survivor空间完成一些安全点下的操作
...
HeapWord* DefNewGeneration::allocate(size_t word_size,
  ...
  // 3. 从From Survivor空间分配内存
  if (result == NULL) {
    // Eden区没有足够的空间,则从From区分配
    // HeapWord* DefNewGeneration::allocate_from_space(size_t size)
